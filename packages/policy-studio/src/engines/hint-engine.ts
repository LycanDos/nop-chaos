...
export class HintEngine {
  ...
  getFieldSuggestions(
    ...
    // 1. 模糊搜索字段
    const matches = this.fuzzySearchFields(input);
  ...
  private fuzzySearchFields(
    ...
    // 搜索实体字段
    this.fieldMapping.entities.forEach((entityMap) => {
      entityMap.fields.forEach((field) => {
        const score = this.calculateMatchScore(input, field);
        if (score > 0) {
          results.push({
            field,
  ...
  parsePath(path: string): FieldInfo | null {
    ...
    // 尝试通过别名查找
    for (const [, field] of this.fieldMapping.globalFields) {
  ...
  getOperatorSuggestions(fieldType?: string): Array<{label: string; value: string; type: string}> {
    ...
    // 根据字段类型添加特定运算符
    if (!fieldType || ['int', 'integer', 'long', 'decimal', 'double', 'float', 'number'].includes(fieldType.toLowerCase())) {
      operators.push(
    ...
    if (!fieldType || fieldType.toLowerCase() === 'string') {
      operators.push(
    ...
    if (!fieldType || fieldType.toLowerCase() === 'boolean') {
      operators.push(
