...
<template>
  <a-collapse-panel key="arg1" header="定时">
    <a-form-item label="类型">
      <a-radio-group v-model="type" placeholder="请选择时间类型">
        <a-radio-button label="时间" value="timeDate" />
        <a-radio-button label="持续" value="timeDuration" />
        <a-radio-button label="循环" value="timeCycle" />
      </a-radio-group>
    </a-form-item>
    ...
    <a-form-item label="持续时间" v-else-if="type === 'timeDuration'">
      <a-input v-model="time" placeholder="请输入持续时间" />
    </a-form-item>
    <a-form-item label="循环时间" v-else-if="type === 'timeCycle'">
      <a-input v-model="time" placeholder="请输入循环时间" />
    </a-form-item>
  </a-collapse-panel>
</template>
